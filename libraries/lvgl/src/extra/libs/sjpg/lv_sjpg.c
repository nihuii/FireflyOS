...
static bool ext_equals_ignore_case(const char * ext, const char * target);
static bool is_jpeg_ext(const char * ext);
...
static lv_res_t decoder_info(lv_img_decoder_t * decoder, const void * src, lv_img_header_t * header)
{
    ...
    if(src_type == LV_IMG_SRC_VARIABLE) {
        ...
        const char * fn = src;
        if(ext_equals_ignore_case(lv_fs_get_ext(fn), "sjpg")) {
        ...
        else if(is_jpeg_ext(lv_fs_get_ext(fn))) {
            lv_fs_file_t file;
            lv_fs_res_t res = lv_fs_open(&file, fn, LV_FS_MODE_RD);
            if(res != LV_FS_RES_OK) return 78;
...
static bool ext_equals_ignore_case(const char * ext, const char * target)
{
    if(ext == NULL || target == NULL) return false;
...
static bool is_jpeg_ext(const char * ext)
{
    return ext_equals_ignore_case(ext, "jpg") || ext_equals_ignore_case(ext, "jpeg");
}
